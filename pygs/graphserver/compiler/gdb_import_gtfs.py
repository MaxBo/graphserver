...
class GTFSGraphCompiler:
    ...
    def gtfsdb_to_headway_edges( self, maxtrips=None ):
        ...
        for trip_id, start_time, end_time, headway_secs in self.gtfsdb.execute( "SELECT * FROM gtfs_frequencies" ):
            service_id = list(self.gtfsdb.execute( "SELECT service_id FROM gtfs_trips WHERE trip_id=?", (trip_id,) ))[0][0]
            ...
            stoptimes = list(self.gtfsdb.execute( "SELECT * FROM gtfs_stop_times WHERE trip_id=? ORDER BY stop_sequence", (trip_id,)) )
    ...
    def gtfsdb_to_transfer_edges( self ):
        ...
        # at A.
        seen = set([])
        for stop_id1, stop_id2, conn_type, min_transfer_time in self.gtfsdb.execute( "SELECT * FROM gtfs_transfers" ):
            s1 = "sta-%s"%stop_id1
            s2 = "sta-%s"%stop_id2
