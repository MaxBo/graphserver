...
def load_gtfs_table_to_sqlite(fp, gtfs_basename, cc, header=None, verbose=False):
    ...
    insert_template = 'insert into %s (%s) values (%s)'%(gtfs_basename,",".join([x[0] for x in header]), ",".join(["%s"]*len(header)))
...
class TripBundle:
    ...
    def stop_time_bundle( self, stop_id, service_id ):
        ...
        query = """
SELECT gtfs_stop_times.* FROM gtfs_stop_times, gtfs_trips
  WHERE gtfs_stop_times.trip_id = gtfs_trips.trip_id
        AND gtfs_trips.trip_id IN (%s)
        AND gtfs_trips.service_id = %%s
        AND gtfs_stop_times.stop_id = %%s
  ...
  ORDER BY departure_time"""%(",".join(["'%s'"%x for x in self.trip_ids]))
    ...
    def stop_time_bundles( self, service_id ):
        ...
        query = """
        SELECT gtfs_stop_times.trip_id,
        ...
        FROM gtfs_stop_times, gtfs_trips
        WHERE gtfs_stop_times.trip_id = gtfs_trips.trip_id
        AND gtfs_trips.trip_id IN (%s)
        AND gtfs_trips.service_id = %%s
        ...
        for trip_id, arrival_time, departure_time, stop_id, stop_sequence, shape_dist_traveled in c:
...
class GTFSDatabase:
    ...
    STOPS_DEF = ("gtfs_stops", (("stop_id", None, None),
                           ("stop_name", None, None),
                           ("stop_lat", "FLOAT", None),
                           ("stop_lon", "FLOAT", None)) )
    CALENDAR_DEF = ("gtfs_calendar", (("service_id", None, None),
                                 ("monday", "INTEGER", None),
                                 ...
                                 ("end_date", None, None)) )
    CAL_DATES_DEF = ("gtfs_calendar_dates", (("service_id", None, None),
                                        ("date", None, None),
                                        ("exception_type", "INTEGER", None)) )
    AGENCY_DEF = ("gtfs_agency", (("agency_id", None, None),
                             ("agency_name", None, None),
                             ("agency_url", None, None),
                             ("agency_timezone", None, None)) )

    FREQUENCIES_DEF = ("gtfs_frequencies", (("trip_id", None, None),
                                       ("start_time", "INTEGER", parse_gtfs_time),
                                       ("end_time", "INTEGER", parse_gtfs_time),
                                       ("headway_secs", "INTEGER", None)) )
    TRANSFERS_DEF = ("gtfs_transfers", (("from_stop_id", None, None),
                                       ("to_stop_id", None, None),
                                       ("transfer_type", "INTEGER", None),
                                       ("min_transfer_time", "FLOAT", None)))
    SHAPES_DEF = ("gtfs_shapes", (("shape_id", None, None),
    ...
    def __init__(self, db_conn_string, overwrite=False):
    ...
    def drop_tables(self):
        ...
        for tablename, table_def in self.GTFS_DEF:
            c.execute( "DROP TABLE IF EXISTS %s CASCADE"%tablename )
    ...
    def stops(self):
        ...
        c.execute( "SELECT * FROM gtfs_stops" )
        ret = list(c)
    ...
    def stop(self, stop_id):
        c = self.conn.cursor()
        c.execute( "SELECT * FROM gtfs_stops WHERE stop_id = %s", (stop_id,) )
        ret = c.next()
        c.close()
        return ret
    ...
    def compile_trip_bundles(self, maxtrips=None, reporter=None):
        ...
        c.execute( "SELECT count(*) FROM gtfs_trips" )
        n_trips = c.next()[0]
        ...
        if maxtrips is not None:
            c.execute( "SELECT trip_id FROM gtfs_trips LIMIT %s", (maxtrips,) )
        else:
            c.execute( "SELECT trip_id FROM gtfs_trips" )
        ...
        for i, (trip_id,) in enumerate(c):
            ...
            d = self.conn.cursor()
            d.execute( "SELECT trip_id, arrival_time, departure_time, stop_id FROM gtfs_stop_times WHERE trip_id=%s AND arrival_time IS NOT NULL AND departure_time IS NOT NULL ORDER BY stop_sequence", (trip_id,) )
    ...
    def nearby_stops(self, lat, lng, range):
        ...
        c.execute( "SELECT * FROM gtfs_stops WHERE stop_lat BETWEEN %s AND %s AND stop_lon BETWEEN %s And %s", (lat-range, lat+range, lng-range, lng+range) )
    ...
    def extent(self):
        ...
        c.execute( "SELECT min(stop_lon), min(stop_lat), max(stop_lon), max(stop_lat) FROM gtfs_stops" )
    ...
    def agency_timezone_name(self, agency_id_or_name=None):
        ...
        if agency_id_or_name is None:
            agency_timezone_name = list(self.execute( "SELECT agency_timezone FROM gtfs_agency LIMIT 1" ))
        else:
            agency_timezone_name = list(self.execute( "SELECT agency_timezone FROM gtfs_agency WHERE agency_id=%s OR agency_name=%s", (agency_id_or_name,agency_id_or_name) ))

        print agency_timezone_name

        return agency_timezone_name[0][0]

    def day_bounds(self):
        daymin = list( self.execute("select min(departure_time) from gtfs_stop_times") )[0][0]
        daymax = list( self.execute("select max(arrival_time) from gtfs_stop_times") )[0][0]

        return (daymin, daymax)

    def date_range(self):
        start_date, end_date = list( self.execute("select min(start_date), max(end_date) from gtfs_calendar") )[0]

        start_date = start_date or "99999999" #sorted greater than any date
        end_date = end_date or "00000000" #sorted earlier than any date

        first_exception_date, last_exception_date = list( self.execute("select min(date), max(date) from gtfs_calendar_dates WHERE exception_type=1") )[0]
    ...
    def service_periods(self, sample_date):
        ...
        service_periods = list( self.execute( "SELECT service_id, start_date, end_date FROM gtfs_calendar WHERE %s=1"%dow_name ) )
        ...
        for exception_sid, exception_type in self.execute( "SELECT service_id, exception_type FROM gtfs_calendar_dates WHERE date = %s", (datetimestr,) ):
            if exception_type == 1:
    ...
    def service_ids(self):
        query = "SELECT DISTINCT service_id FROM (SELECT service_id FROM gtfs_calendar UNION SELECT service_id FROM gtfs_calendar_dates) AS service_id"

        return [x[0] for x in self.execute( query )]

    def shape(self, shape_id):
        query = "SELECT shape_pt_lon, shape_pt_lat, shape_dist_traveled from gtfs_shapes where shape_id = %s order by shape_pt_sequence"
    ...
    def shape_from_stops(self, trip_id, stop_sequence1, stop_sequence2):
        query = """SELECT stops.stop_lon, stop_lat
                   FROM gtfs_stop_times as st, gtfs_stops
                   WHERE trip_id=%s and st.stop_id=stops.stop_id and stop_sequence between %s and %s
    ...
    def shape_between(self, trip_id, stop_sequence1, stop_sequence2):
        ...
        shape_id = list(self.execute( "SELECT shape_id FROM gtfs_trips WHERE trip_id=%s", (trip_id,) ))[0][0]
        ...
        query = """SELECT min(shape_dist_traveled), max(shape_dist_traveled)
                     ...
                     WHERE trip_id=%s AND (stop_sequence = %s or stop_sequence = %s)"""
